...
# Завантажуємо необхідні бібліотеки для аналізу та візуалізації
...
# Ми побудуємо графіки гістограми без урахуванням нулів, так 
# ми можемо детальніше побачити, як розподіляються результати лише серед тих, 
# хто подолав поріг. У такому випадку не буде сильного 
...
# Гістограма розподілу балів з української мови без нулів (тільки ті, хто подолав поріг)
...
mean_score <- mean(data_filtered$UkrBlockBall100, na.rm = TRUE)
sd_score <- sd(data_filtered$UkrBlockBall100, na.rm = TRUE)

# Створення даних для кривої нормального розподілу
x_vals <- seq(min(data_filtered$UkrBlockBall100, na.rm = TRUE),
              max(data_filtered$UkrBlockBall100, na.rm = TRUE),
              length.out = 100)

bin_width <- (max(data_filtered$UkrBlockBall100, na.rm = TRUE) -
                min(data_filtered$UkrBlockBall100, na.rm = TRUE)) / 30

normal_curve <- data.frame(
  x = x_vals,
  y = dnorm(x_vals, mean = mean_score, sd = sd_score) * 
    length(na.omit(data_filtered$UkrBlockBall100)) * bin_width
)

# Побудова гістограми з кривою нормального розподілу
ggplot(data_filtered, aes(x = UkrBlockBall100)) +
  geom_histogram(fill = "steelblue", color = "black", bins = 30) +
  geom_line(data = normal_curve, aes(x = x, y = y), color = "red", size = 1) +
...
# Гістограма розподілу балів з математики
# Створюємо підмножину даних без нульових значень для математики
data_filtered_math <- my_data[my_data$MathBlockBall100 > 0, ]

not_passed_math <- sum(my_data$MathBlockBall100 == 0, na.rm = TRUE)
total_math <- sum(!is.na(my_data$MathBlockBall100))
percent_not_passed_math <- round(not_passed_math / total_math * 100, 1)

ggplot(data_filtered_math, aes(x = MathBlockBall100)) +
  geom_histogram(bins = 30, fill = "seagreen", color = "white", alpha = 0.7) +
  labs(title = "Розподіл балів з математики (без нулів)",
       subtitle = paste(not_passed_math, "учасників (", percent_not_passed_math, "%) не подолали поріг"),
       x = "Бали",
       y = "Кількість учасників") +
  theme_minimal()

# На гістограмі ми спостерігаємо чітку бімодальність: два локальних модуси 
# приблизно на 110–120 балах та на 150 балах. Це свідчить про наявність у 
# вибірці двох когорт учасників — «майже порогових» та «впевнено високих» 
# результатів. Розмах даних (мінімум ≈105, максимум ≈195) вказує на значну 
# дисперсію, підсилену наявністю довшого правостороннього хвоста, тобто 
# позитивної асиметрії. Така асиметрія часто супроводжується експоненціальною 
# чи важкою «хвостовою» поведінкою, коли кількість спостережень поступово 
# зменшується в напрямку екстремальних значень. Інтерквартильний розмах (IQR) 
# охоплює приблизно середину між двома пиками, що демонструє широкий 
# внутрішньоквартильний діапазон і високу варіацію результатів. Крім того, 
# поодинокі спостереження понад 180 балів можна розглядати як викиди або 
# «екстреми» - невеликі за кількістю, але суттєво впливають на оцінку куртозису 
# (збільшують «гостроту» хвостів розподілу).

# Гістограма розподілу балів з історії України
# Створюємо підмножину даних без нульових значень для історії
data_filtered_hist <- my_data[my_data$HistBlockBall100 > 0, ]

not_passed_hist <- sum(my_data$HistBlockBall100 == 0, na.rm = TRUE)
total_hist <- sum(!is.na(my_data$HistBlockBall100))
percent_not_passed_hist <- round(not_passed_hist / total_hist * 100, 1)

ggplot(data_filtered_hist, aes(x = HistBlockBall100)) +
  geom_histogram(bins = 30, fill = "tomato", color = "white", alpha = 0.7) +
  labs(title = "Розподіл балів з історії України (без нулів)",
       subtitle = paste(not_passed_hist, "учасників (", percent_not_passed_hist, "%) не подолали поріг"),
       x = "Бали",
       y = "Кількість учасників") +
  theme_minimal()

# Гістограма розподілу балів з історії України демонструє унімодальний, близький 
# до симетричного розподіл із чітким центральним модусом на рівні ≈145 балів. 
# Варіація значень у межах 130–160 балів досить мала, а дисперсія та стандартне 
# відхилення нижчі, ніж у математиці. Хвости розподілу коротші і менш «товсті», 
# що вказує на помірну куртозис ближче до нормальної форми. Наявність слабкої 
# позитивної асиметрії злегка зсуває середнє в праву сторону, проте розкид даних 
# мінімальний, і майже відсутні виразні викиди. Така однорідність результатів 
# свідчить про те, що група студентів має приблизно однаковий рівень підготовки 
# з історії, на відміну від більш гетерогенного профілю з математики.

# Щільність розподілу балів з української мови за статтю
ggplot(data_filtered, aes(x = UkrBlockBall100, fill = factor(SexTypeName))) +
  geom_density(alpha = 0.5) +
  labs(title = "Щільність розподілу балів з української мови за статтю (без нулів)",
       x = "Бали",
       y = "Щільність",
       fill = "Стать") +
  scale_fill_brewer(palette = "Set1", labels = c("Жіноча", "Чоловіча")) +
  theme_minimal()

# Коли ми наклали криві щільності для жіночої та чоловічої підгруп, одразу впадає 
# в око, що обидві крива мають довгий правий хвіст, проте в жінок він трохи 
# "товстіший": середньомодальне значення їхніх балів знаходиться ближче до 155, 
# тоді як у чоловіків пік трохи зміщений ліворуч, приблизно на 145–150. Це говорить 
# про те, що жінки у середньому показали трохи вищі результати, ніж чоловіки. 
# Обидві криві демонструють позитивну асиметрію, але у жіночій вибірці асиметрія 
# виражена трохи сильніше, що відображає більшу кількість екстремально високих 
# результатів. Накладення кривих показує високу міру подібності форм розподілів, 
# але невеликі зсуви мод свідчать про статистично помітні відмінності між групами.

# Щільність розподілу балів з української мови за типом території
ggplot(data_filtered, aes(x = UkrBlockBall100, fill = factor(TerTypeName))) +
  geom_density(alpha = 0.5) +
  labs(title = "Щільність розподілу балів з української мови за типом території (без нулів)",
       x = "Бали",
       y = "Щільність",
       fill = "Тип території") +
  scale_fill_brewer(palette = "Dark2", 
                    labels = c("Інша країна", "Місто", "Селище, село")) +
  theme_minimal()

# Аналіз розподілу для трьох категорій за типом території (місто, селище/село, 
# інша країна) також дає цікаві спостереження. Усі три криві концентруються 
# навколо одного чіткого максимуму біля 145 балів, та йдуть у широкий правий 
# хвіст до майже 200. При цьому щільність для міських учасників найгостріша, 
# що вказує на найменший розкид їхніх оцінок: більшість учнів із міст здобула 
# бали дуже близько до центру розподілу з незначними відхиленнями. У сільській 
# підгрупі ми бачимо трохи ширший "фунт" і довший лівий край, що свідчить про 
# більшу змінність і більше учасників із нижчими оцінками (близько 120-130). 
# Підгрупа тих, хто складав за межами країни, розташована посередині: їхній 
# розподіл майже ідентичний міському в центральній зоні, але з дещо товстішим 
# правим хвостом - тобто серед закордонних здобувачів більше тих, хто отримав 
# екстремально високі бали. Загалом, хоча механічно форма всіх трьох кривих дуже 
# схожа, невеликі відмінності в гостроті піків і довжині хвостів допомагають нам 
# виявити тонкі соціально-демографічні тенденції у складанні екзамену.

# Порівняння розподілів балів з різних предметів (українська мова, математика, історія України, англійська мова)
# Створюємо відфільтрований набір даних без нульових значень для всіх предметів
subjects_data_filtered <- my_data %>%
  filter(UkrBlockBall100 > 0, MathBlockBall100 > 0, HistBlockBall100 > 0, EngBlockBall100 > 0) %>%
  select(UkrBlockBall100, MathBlockBall100, HistBlockBall100, EngBlockBall100) %>%
  pivot_longer(cols = everything(), names_to = "Subject", values_to = "Score") %>%
  mutate(Subject = factor(Subject, levels = c("UkrBlockBall100", "MathBlockBall100", "HistBlockBall100", "EngBlockBall100"),
                          labels = c("Українська мова", "Математика", "Історія України", "Англійська мова")))

ggplot(subjects_data_filtered, aes(x = Subject, y = Score, fill = Subject)) +
  geom_boxplot(alpha = 0.7) +
  labs(title = "Порівняння результатів НМТ з різних предметів (без нулів)",
       x = "Предмет",
       y = "Бали") +
...
# На діаграмі загального розподілу балів із чотирьох предметів - української мови, 
# математики, історії України та англійської мови - видно, що медіани для мовних 
# дисциплін (Українська - ≈152, Англійська - ≈150) лежать трохи вище, ніж для 
# Історії України (≈148) і Математики (≈145). Це свідчить про те, що в середньому 
# учасники НМТ демонструють кращі результати з мов, ніж із точних або суспільних наук.
# По-друге, міжквартильні розмахи (IQR) у математики виявилися найкомпактнішими, 
# що означає відносно однорідний «середній клас» результатів - більшість учнів, 
# які склали математику, набрали близько 140-150 балів. У той же час історія 
# України має найбільший IQR, тобто в цій дисципліні «типові» оцінки розкидані 
# ширше, що говорить про різнорідність підготовки. Українська та англійська мови 
# демонструють проміжну варіабельність, але обидві мають трохи ширший розкид, 
# ніж математика.
# Третій важливий момент - «вуса» діаграм охоплюють повний можливий діапазон 
# шкали від 100 до 200 балів (включно). Оскільки 100 і 200 - це офіційні межі 
# оцінювання, їх слід розглядати не як аномальні викиди, а як природні граничні 
# значення системи. Водночас внутрішні «викиди» - тобто окремі точки, які виходять 
# за межі 1,5×IQR, - показують рідкісні низькі (ближче до 100-110) та високі 
# (понад 190) результати. Найбільша кількість високих оцінок (над 190) 
# спостерігається в історії, тоді як у математиці більш виражений хвіст із 
# низькими результатами.
# У підсумку, порівняння підкреслює, що хоча за центральними тенденціями мовні 
# предмети трохи випереджають інші, найстабільніший розподіл у математиці, а 
# найбільша внутрішня різнорідність - в історії України.

# Коробкова діаграма розподілу балів з української мови без нулів
...
#У цій регресійній моделі ми розглядаємо лише учасників, які подолали пороговий 
# бал з української мови.Ми також враховуємо, що значення 0 в інших предметах 
# (математика, історія, англійська) також означає непроходження порогового бала.
# Тому ця модель відображає взаємозв'язки між балами для тих учасників, які 
...
# Графік результатів лінійної регресії
...
ggplot(plot_data, aes(x = Actual, y = Predicted)) +
  ...
  labs(title = "Результати лінійної регресії",
       subtitle = paste("R² =", round(linear_results$R_squared, 3), 
                        ", RMSE =", round(linear_results$RMSE, 3)),
       x = "Фактичні бали",
       y = "Прогнозовані бали") +
  theme_minimal() +
...
# Візуалізація структури дерева рішень
if (!requireNamespace("rpart.plot", quietly = TRUE)) {
  install.packages("rpart.plot")
}
library(rpart.plot)

# Отримуємо безпосередньо об'єкт rpart з об'єкта train
final_tree <- rpart_model$finalModel

# Візуалізація дерева рішень
rpart.plot(final_tree, 
           box.palette = "RdBu",     # Палітра кольорів
           shadow.col = "gray",      # Колір тіні
           nn = TRUE,                # Показувати кількість спостережень у вузлах
           main = "Дерево рішень для прогнозування балів з української мови")

library(rpart.plot)

png("C:/analis/decision_tree_hi.png", width  = 9500,height = 2500,res = 300)
...
# Графік результатів дерева рішень
plot_data_tree <- data.frame(
  Actual = y_test,
  Predicted = rpart_predictions
)

library(grid)

ggplot(plot_data_tree, aes(x = Actual, y = Predicted)) +
  geom_point(alpha = 0.5, color = "darkred") +
  geom_abline(intercept = 0, slope = 1, color = "blue", linetype = "dashed") +
  labs(title = "Результати регресійного дерева рішень",
       subtitle = paste("R² =", round(rpart_results$R_squared, 3), 
                      ", RMSE =", round(rpart_results$RMSE, 3)),
       x = "Фактичні бали",
       y = "Прогнозовані бали") +
  theme_minimal() +
  xlim(100, 200) + ylim(100, 200) +
  geom_smooth(method = "loess", se = TRUE, color = "darkgreen", fill = "lightgreen", alpha = 0.3)
...
# Візуалізація важливості змінних у моделі Random Forest
if(!requireNamespace("vip", quietly = TRUE)) {
  install.packages("vip")
}
library(vip)

# Візуалізація важливості змінних
vip(rf_model, 
    num_features = 5,  # показати всі 5 змінних
    bar = TRUE,        # стовпчикова діаграма
    horizontal = TRUE, # горизонтальна орієнтація
    aesthetics = list(fill = "steelblue", color = "black")) +
  labs(title = "Важливість змінних у моделі Random Forest",
       subtitle = "На основі зниження точності при перестановці значень") +
  theme_minimal()

# Графік результатів Random Forest
...
ggplot(plot_data_rf, aes(x = Actual, y = Predicted)) +
  ...
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(title = "Результати Random Forest",
       subtitle = paste("R² =", round(rf_results$R_squared, 3), 
                      ", RMSE =", round(rf_results$RMSE, 3)),
       x = "Фактичні бали",
       y = "Прогнозовані бали") +
  theme_minimal() +
  xlim(100, 200) + ylim(100, 200) +
...
ggplot(plot_data_xgb, aes(x = Actual, y = Predicted)) +
  ...
  geom_abline(intercept = 0, slope = 1, color = "red", linetype = "dashed") +
  labs(title = "Результати XGBoost",
       subtitle = paste("R² =", round(xgb_results$R_squared, 3), 
                      ", RMSE =", round(xgb_results$RMSE, 3)),
       x = "Фактичні бали",
       y = "Прогнозовані бали") +
  theme_minimal() +
  xlim(100, 200) + ylim(100, 200) +
...
# Візуалізація важливості змінних для XGBoost
importance_matrix <- xgb.importance(colnames(X_train), model = xgb_model)
xgb.plot.importance(importance_matrix, top_n = 5, main = "Важливість змінних у моделі XGBoost")
...
# Графік результатів логістичної регресії - ROC крива
library(ROCR)
pred_obj <- prediction(log_probs, y_test_class)
perf_obj <- performance(pred_obj, "tpr", "fpr")
auc_value <- performance(pred_obj, "auc")@y.values[[1]]

# Створення даних для побудови кривої
roc_data <- data.frame(
...
ggplot(roc_data, aes(x = FPR, y = TPR)) +
  ...
  annotate("text", x = 0.75, y = 0.25, 
           ...
           size = 5, color = "red") +
  labs(title = "ROC крива для логістичної регресії",
       subtitle = "Результати класифікації високих балів з української мови",
       x = "False Positive Rate (1 - Специфічність)",
       y = "True Positive Rate (Чутливість)") +
  theme_minimal() +
  coord_equal() +
...
# Тренування моделі з використанням факторних змінних
...
# Візуалізація структури дерева класифікації
tree_model <- rpart_model_class$finalModel
rpart.plot(tree_model, 
           box.palette = "GnBu",     # Палітра кольорів
           shadow.col = "gray",      # Колір тіні
           nn = TRUE,                # Показувати кількість спостережень у вузлах
           main = "Дерево рішень для класифікації високих балів з української мови")

# ROC-крива для дерева рішень
dt_pred_obj <- prediction(rpart_probs_class, y_test_class)
dt_perf_obj <- performance(dt_pred_obj, "tpr", "fpr")
dt_auc_value <- performance(dt_pred_obj, "auc")@y.values[[1]]

# Створення даних для побудови кривої
dt_roc_data <- data.frame(
  FPR = dt_perf_obj@x.values[[1]],
  TPR = dt_perf_obj@y.values[[1]]
)

# Побудова ROC кривої для дерева рішень
ggplot(dt_roc_data, aes(x = FPR, y = TPR)) +
  geom_line(color = "darkred", size = 1.2) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed", color = "darkgrey") +
  annotate("text", x = 0.75, y = 0.25, 
           label = paste("AUC =", round(dt_auc_value, 4)), 
           size = 5, color = "blue") +
  labs(title = "ROC крива для дерева рішень",
       subtitle = "Класифікація високих балів з української мови",
       x = "False Positive Rate (1 - Специфічність)",
       y = "True Positive Rate (Чутливість)") +
  theme_minimal() +
  coord_equal() +
...
## ROC-крива
rf_pred_obj  <- ROCR::prediction(rf_probs_class, y_test_class)
rf_perf_obj  <- ROCR::performance(rf_pred_obj, "tpr", "fpr")
rf_auc_value <- ROCR::performance(rf_pred_obj, "auc")@y.values[[1]]

rf_roc_df <- data.frame(
  FPR = rf_perf_obj@x.values[[1]],
  TPR = rf_perf_obj@y.values[[1]]
)

ggplot(rf_roc_df, aes(FPR, TPR)) +
  geom_line(colour = "darkgreen", size = 1.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "grey50") +
  annotate("text", x = .75, y = .25,
           label = paste("AUC =", sprintf("%.4f", rf_auc_value)),
           colour = "blue", size = 5) +
  labs(title     = "ROC-крива для Random Forest",
       subtitle  = "Класифікація високих балів з української мови",
       x = "False Positive Rate (1 – специфічність)",
       y = "True Positive Rate (чутливість)") +
...
## важливість ознак
rf_imp <- data.frame(
  Feature = names(rf_model_class$finalModel$variable.importance),
  Importance = rf_model_class$finalModel$variable.importance
)
...
ggplot(rf_imp,
  ...
  coord_flip() +
  labs(title    = "Важливість ознак у Random Forest",
       x        = NULL,
...
## ROC-крива
xgb_pred_obj  <- ROCR::prediction(xgb_probs_class, y_test_class)
xgb_perf_obj  <- ROCR::performance(xgb_pred_obj, "tpr", "fpr")
xgb_auc_value <- ROCR::performance(xgb_pred_obj, "auc")@y.values[[1]]

xgb_roc_df <- data.frame(
  FPR = xgb_perf_obj@x.values[[1]],
  TPR = xgb_perf_obj@y.values[[1]]
)

ggplot(xgb_roc_df, aes(FPR, TPR)) +
  geom_line(colour = "forestgreen", size = 1.2) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", colour = "grey50") +
  annotate("text", x = .75, y = .25,
           label = paste("AUC =", sprintf("%.4f", xgb_auc_value)),
           colour = "red", size = 5) +
  labs(title     = "ROC-крива для XGBoost",
       subtitle  = "Класифікація високих балів з української мови",
       x = "False Positive Rate (1 – специфічність)",
       y = "True Positive Rate (чутливість)") +
  theme_minimal() + coord_equal()
...
# Отримання важливості ознак для XGBoost
xgb_imp <- xgb.importance(model = xgb_model_class)

imp_df <- xgb_imp[order(-xgb_imp$Gain), ][1:5, ]   # топ-5 за Gain
ggplot(imp_df, aes(x = reorder(Feature, Gain), y = Gain)) +
  geom_col(fill = "forestgreen") +
  coord_flip() +
  labs(title = "Важливість ознак у XGBoost",
       x = NULL,
       y = "Gain") +
  theme_minimal()
